...
const TAGS = ['ALL', 'WEB', 'DATA', 'COMPOSE', 'AGENT', 'PERSONA', 'AUDIO', 'VIDEO'];
      ...
      <section style={{ borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1152px', margin: '0 auto', padding: '80px 40px' }}>
          ...
          <p style={{ fontSize: '1rem', color: 'var(--text-2)', maxWidth: '560px', lineHeight: 1.7 }}>
            Any browser, agent, or runtime can discover, pay for, and execute skills — no accounts, no API keys. Just a Solana wallet and USDC.
          </p>
        ...
        <div style={{ maxWidth: '1152px', margin: '0 auto', padding: '32px 40px' }}>
          <div className="flex flex-col sm:flex-row gap-3 mb-5">
            <div className="flex-1 flex items-center gap-3 px-5 py-4 rounded-xl"
              ...
              <span style={{ color: 'var(--green)', fontFamily: "'Share Tech Mono', monospace", fontSize: '16px', flexShrink: 0 }}>
                $
              </span>
              <input
                type="text"
                ...
                placeholder="execute --skill scrape  ·  or enter a tag to filter"
                style={{
                  flex: 1, background: 'transparent', border: 'none', outline: 'none',
                  color: 'var(--text-1)', fontFamily: "'Share Tech Mono', monospace",
                  fontSize: '16px', caretColor: 'var(--green)',
                }}
              />
            </div>
            <button
              ...
              >
                ...
                <div className="flex items-start justify-between gap-4 mb-5">
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontFamily: "'VT323', monospace", fontSize: '32px', color: 'var(--green)', lineHeight: 1 }}>
                      ${skill.price.toFixed(3)}
                    </div>
                    <div style={{ fontSize: '11px', color: 'var(--text-3)', marginTop: '2px' }}>USDC / call</div>
                  </div>
                </div>

                {/* Description */}
                <p style={{ fontSize: '14px', color: 'var(--text-2)', lineHeight: 1.65, marginBottom: '16px', flexGrow: 1 }}>
                  {skill.description}
                </p>
                ...
                <div className="flex gap-3" style={{ borderTop: '1px solid var(--border)', paddingTop: '16px' }}>
                  <button
                    ...
                    onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg-card-2)'; e.currentTarget.style.color = 'var(--text-1)'; }}
                    onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = copied === skill.id ? 'var(--green)' : 'var(--text-2)'; }}
                  >
                    {copied === skill.id ? '✓ Copied' : 'Copy MCP'}
                  </button>
                  <Link
                    href={`/play/${skill.id}`}
                    style={{
              ...
              <div>
                {executions.map((exec) => (
                  <div key={exec.id} className="slide-in flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-4"
                    ...
                    <div className="flex items-center gap-4">
                      <span style={{ background: 'rgba(20,241,149,0.12)', color: 'var(--green)', fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '999px', fontFamily: "'Share Tech Mono', monospace', border: '1px solid rgba(20,241,149,0.2)'" }}>
                        OK
                      </span>
                      <div>
